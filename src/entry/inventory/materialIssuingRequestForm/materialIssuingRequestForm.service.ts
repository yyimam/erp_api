...
@Injectable()
export class MaterialIssuingRequestFormService {
  constructor(
    @InjectModel(BillingMaster)
    private readonly BillingMasterModel: typeof BillingMaster,
  ) { }

  async create(CreateMaterialIssuingRequestFormDto: CreateMaterialIssuingRequestFormDto): Promise<BillingMaster> {
    let t: {} = CreateMaterialIssuingRequestFormDto
    return this.BillingMasterModel.create(t);
  }

  async update(code: string, UpdateMaterialIssuingRequestFormDto: CreateMaterialIssuingRequestFormDto): Promise<any> {
    let t: {} = UpdateMaterialIssuingRequestFormDto;
  ...
  async findOne(code: string): Promise<BillingMaster> {
    return this.BillingMasterModel.findOne({
      include: [ 
      {model: BillingDetails, include: [{ model: Product,as: '_itemcode', attributes: [['name', 'label'], ['code', 'value']] }, { model: Product,as: '_fitemcode', attributes: [['name', 'label'], ['code', 'value']] }], attributes:['idno', 'pidno', 'itemcode', 'qty','description', 'QC_Remarks', 'fitemcode']},
      // { model: AC, attributes: [['title', 'label'], ['acode', 'value']] },
      // { model: WarehouseList, attributes: [['name', 'label'], ['code', 'value']] }
    ],
      where: { idno: code },
