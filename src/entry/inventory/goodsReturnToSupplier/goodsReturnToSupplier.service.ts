...
@Injectable()
export class GoodsReturnToSupplierService {
  constructor(
    @InjectModel(BillingMaster)
    private readonly BillingMasterModel: typeof BillingMaster,
  ) { }

  async create(CreateGoodsReturnToSupplierDto: CreateGoodsReturnToSupplierDto): Promise<BillingMaster> {
    let t: {} = CreateGoodsReturnToSupplierDto
    return this.BillingMasterModel.create(t);
  }

  async update(code: string, UpdateGoodsReturnToSupplierDto: CreateGoodsReturnToSupplierDto): Promise<any> {
    let t: {} = UpdateGoodsReturnToSupplierDto;
